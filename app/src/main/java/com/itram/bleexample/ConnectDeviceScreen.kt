...
import com.itram.bleexample.MainActivity.Companion.CHARACTERISTIC_UUID
import com.itram.bleexample.MainActivity.Companion.SERVICE_UUID
...
fun ConnectDeviceScreen(device: BluetoothDevice, onClose: () -> Unit) {
    ...
    val service by remember(state?.services) {
        state?.services?.forEach {
            Log.d("ConnectDeviceScreen", "Service: ${it.uuid}")
        }
    ...
    val characteristic by remember(service) {
        service?.characteristics?.forEach {
            Log.d("ConnectDeviceScreen", "Characteristics: ${it.uuid}")
        }
...
private fun sendData(
    ...
    val data = "Eve Weather D89E".toByteArray()
    ...
    DisposableEffect(lifecycleOwner, device) {
        ...
        val callback = object : BluetoothGattCallback() {
            ...
            private fun doOnRead(value: ByteArray) {
                Log.d("ConnectDeviceScreen", "Message received: ${value.decodeToString()}")
